...
from schemas.posts import CreatePost, PostBase
...
def test_posts_sent(post_post:CreatePost, db:Session = Depends(get_db)):
...
def delete_test_post(id:int, db:Session = Depends(get_db)):
    ...
    deleted_post = db.query(Post).filter(modelsPost.id == id)
...
@router.put('/posts/{id}', response_model=CreatePost)
def update_test_post(update_post:PostBase, id:int, db:Session = Depends(get_db)):
